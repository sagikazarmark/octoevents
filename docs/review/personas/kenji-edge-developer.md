...
2. **Sans-I/O path.** With no default features, write a function from an `http::HeaderMap` and body bytes to a status code using the signed envelope constructor, the response-status mapping, and a dispatcher or plain match for `release.published`. Forward every envelope by serializing it. Record: how you ran an async dispatch from a sync path; whether the docs told you what the receiver does that this path does not; whether public header-name constants exist and what the docs say they are for.
