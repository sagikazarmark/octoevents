...
`unrepresentable.json` is deliberately synthetic valid JSON that octocrab cannot decode, used to test that the `always` tier, event handlers over consumer views or `()`, and a strict fallback run without it.
